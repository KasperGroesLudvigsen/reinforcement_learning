...
class Paparazzi:
    """ Generates paparazzi objects """
    
    def __init__(self, env_size):
        
        # Creating arrays that will move paps north, south, east and west
        
        self.south_arr = np.array([-1, 0])
        self.east_arr = np.array([0, 1])
        self.west_arr = np.array([0, -1])
        
        self.location, self.travel_direction = self.create_pap(env_size)
        
        self.direction_arr = self.create_directional_arr()
    ...
    def create_directional_arr(self):
        """Creating array that will move paps north, south, east and west"""
        if self.travel_direction == "E":
            return np.array([0, 1])
        if self.travel_direction == "W":
            return np.array([0, -1])
        if self.travel_direction == "N":
            return np.array([1, 0])
        if self.travel_direction == "S":
            return np.array([-1, 0])         
